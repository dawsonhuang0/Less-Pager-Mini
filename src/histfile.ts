...
export function loadHistory(): void {
  ...
  for (const line of lines.slice(1)) {
    if (line.startsWith('.')) {
      inSearch = line === SEARCH_SECTION;
    } else if (inSearch && line.startsWith('"') && line.length > 1) {
      patterns.push(line.slice(1));
    }
  }

  search.history = patterns.slice(-historyLimit());
  loadedKey = JSON.stringify(search.history);
}
...
export function saveHistory(): void {
  ...
  try {
    ...
    if (lines[0].startsWith(FIRST_LINE)) {
      ...
      for (const line of lines.slice(1)) {
        if (line.startsWith('.')) keep = line !== SEARCH_SECTION;
  ...
  try {
    ...
    fs.writeFileSync(file, FIRST_LINE + '\n' + section + others, {
      mode: 0o600,
    });
    loadedKey = JSON.stringify(entries);
  } catch {
