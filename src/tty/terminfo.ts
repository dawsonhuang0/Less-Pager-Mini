 ...
 * every form in the databases we have seen for cm; anything else is
 * returned unsubstituted, which screen.c treats as a missing capability.
 */
export const tgoto = (cap: string, col: number, row: number): string => {
  let out = '';
  const params = [row, col];  /* terminfo cm is row-first */
  let inc = 0;
  const stack: number[] = [];

  for (let i = 0; i < cap.length; i++) {
    if (cap[i] !== '%') { out += cap[i]; continue; }
    i++;
    switch (cap[i]) {
    case '%': out += '%'; break;
    case 'i': inc = 1; break;
    case 'p': {
      const n = parseInt(cap[++i], 10);
      stack.push(params[n - 1] + inc);
      break;
    }
    case 'd': out += String(stack.pop() ?? 0); break;
    case '2': out += String(stack.pop() ?? 0).padStart(2, '0'); break;
    case '3': out += String(stack.pop() ?? 0).padStart(3, '0'); break;
    case 'c': out += String.fromCharCode(stack.pop() ?? 0); break;
    default:
      /* A construct we do not implement: hand back the raw string. */
      return cap;
    }
  }
  return out;
...
/*
 * The terminal's name list, for diagnostics.
 */
export const terminfo_names = (): string[] => ti === null ? [] : ti.names;
