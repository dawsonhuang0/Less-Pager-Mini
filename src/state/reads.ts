 ...
 * og's `ch_get` reaching an `iread` rather than answering from its
 * buffer pool.
 *
 * This is the whole of og's poll condition. check_poll (os.c:303) runs
 ...
 * the user bursts.
 *
 * That is why og keeps its ":" on short content and drops it on a big
...
/** og's iread: the source was touched, so og would have polled here. */
export function sourceRead(): void {
  readNow = true;
}

/** Starts a new command's watch, like og entering commands(). */
...
/**
 * Whether og would have polled the tty for this command or the one
 * before it.
 *
