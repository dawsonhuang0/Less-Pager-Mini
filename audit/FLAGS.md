...
| F06 | `254fefb` [3153] | An unterminated OSC sequence must have the ENTIRE sequence removed, not be closed with a synthesised ST. | **FIX** — we passed it through RAW (older than either less behaviour), leaking the sequence and the text it swallowed. Fixed in `helpers.ts` transformLine. Verified vs less on `/tmp/osc-unterm.txt`. |
...
| F10 | `c654d98` [3132] | Emit the null OSC 8 sequence at end of line only when actually MID-LINK, not when any link appeared anywhere in the line. | **GAP** — measured: on an unterminated OSC 8, less ends the line with `ESC[m ESC]8;; ESC\\` (attr reset + null OSC 8 closing the link); we emit neither. So we never close an open link at EOL. Needs `in_osc8_link` state in the transform. |
| F11 | `4e4dce3` [3121] | `G` at the end of &-filtered input must still ring the bell when the file's last line is filtered OUT (#774). | **FIX** we never belled. less's jump_forw bells when `bot_pos == end_pos || (bot_pos == soft_eof && soft_eof != NULL_POSITION)` (jump.c:44) — the second half is the filtered end, which bottom+1 never reaches when the real last line is filtered away. Verified: less and we now bell on the same step. NOTE the condition is NOT "EOF is displayed": on a screen ending in tildes bottom+1 is NULL_POSITION and less does not bell, so the screen must be FULL too — my first attempt used mode.EOF alone and regressed the short-file case. |
...
| F27 | `930053c` [3026] | `?O` prompt conditional: true when an OSC 8 link is selected. | **FIX** we hardcoded `false` with a stale "OSC 8 links are not supported" comment. Now `selectedOsc8() !== null`; verified vs less with `-P'?OSELECTED:none.'` — both print `none` then `SELECTED` after `^O^N`. |
...
| F30 | `e5e52da` [3020] | The numeric argument to `m` and `M` changed meaning: it is now the FILE line number, not the screen line number (#736). | **OK** `jumping.ts:702-725` resolves `lineNum` through `linePosition()` and sets `row: lineNum - 1`, with less's "Cannot find line number N" error when it does not resolve. |
...
| F32 | `43957b9` [3001] | The long prompt must reserve a column for the `&` indicator, or it wraps onto a second line (#749). | **OK** probed `-M` with an `&` filter and a 68-character filename at width 80, so the long prompt sits right at the edge: identical to less. |
...
| F35 | `d6204d4` [2989] | OSC 8 opener: `-` prefix suppresses echoing the command, `^P` prefix suppresses the "link done" message. | **OK** ours matches less twice over: `handler === '-'` falls back to `_ANY` exactly like `strcmp(handler,"-")==0` (search.c:1964), `\x10` sets `done = null` like `CONTROL('P')` (search.c:1973), and the "No handler for ... link type" text agrees. |
...
| F38 | `640adf1` [2982] | Forced BACKWARD scrolling stops when the first file line reaches the screen BOTTOM, mirroring forced forward stopping at the top. | **OK** probed `G` then 45×`K` on both a plain 40-line file and a 6-line wrapped one: identical to less. |
| F39 | `feb6a00` [2976] | No unintentional file wrap when force-scrolling with `-c`. | **FIX (adjacent)** probing this family found a real bug of our own: `--past-eof` never reached the stream engine's FORWARD path. less's `forward()` opens `if (past_eof) force = TRUE` (forwback.c:479); ours consulted the option only in `backwardFrom`, so a forward move still stopped at the last screenful. Measured: at step 18 of 40 `j`s less sits on line-018 and we sat on line-017. Fixed in `fileInput.forward`; the `-c` wrap claim itself is still unverified. |
| F40 | `1a5f5e6` [2975] | Forced forward scrolling must stop when the LAST file line reaches the TOP; the old check was wrong. | **OK** probed 45×`J` on plain and wrapped files: identical to less. Note less's test counts SCREEN rows (`empty_lines(3, sc_height-1)` with rows 0-2 non-empty), which the wrapped probe exercises. |
| F41 | `778e1515` [2973] | `pos_rehead()` takes a flag: adjusting `hshift` is WRONG when the rehead is caused by a horizontal shift, or the shift jumps unexpectedly. | **OK** we already split it the way less did: `jumping.ts:345 posRehead()` does NOT touch the shift (= `pos_rehead(FALSE)`, used by LSHIFT/RSHIFT/LLSHIFT/RRSHIFT and the wheel), while `-S` does the hshift adjustment inline in `options/chop-long-lines.ts` (= `pos_rehead(TRUE)`, less's only such caller, optfunc.c:340). |
| F42 | `a8b1c3b` [2971] → `778e1515` | Horizontal DRAG: less calls `pos_rehead(FALSE)` before adjusting hshift (decode.c:666). (The `chopline = TRUE` hack this commit added was REMOVED by 778e1515 once the flag existed — do not port the hack.) | **FIX (UNMEASURED)** our HDRAG branch adjusted `config.col` without re-heading; added `posRehead()`. Transcription of less's call sequence, **not verified against the binary**: the pty harness delivers keys one byte per step, so a mouse sequence is echoed as typed text and never becomes a mouse event. Needs a harness that writes a byte GROUP in one go before this can be called OK. |
...
| F44 | `23f8672` [2952] | A tags-file line number of 0 is INVALID and the tag is skipped, not dereferenced as a pattern (segfault). | **OK** `tags.ts:206` `if (!linenum) continue;` is less's `if (n == 0) continue;`. |
...
| F54 | `2928` `543eb06` | `-DT` formats tilde lines. | **OK** `color.ts:36` maps `T: 'tilde'`; less's `optfunc.c:669` `case 'T': return AT_COLOR_TILDE`. |
| F55 | `2938` `b8f9444` | `--use-backslash` status messages were REVERSED (#735). | **OK** ours reads "Don't use backslash escaping..." then "Use backslash escaping...", matching less opttbl.c:551-552 in that order. |
...
| # | commit | what the message claims | status |
|---|---|---|---|
| F56 | `038ccd9` [2907] | `bin_file` must stop its binary check 4 bytes before the end of its 256-byte buffer, or a truncated UTF-8 sequence reads as malformed. | **OK** built a file with a 4-byte U+1F600 at offset 253 so its tail crosses byte 256: less and we agree, neither treats it as binary. |
| F57 | `18db5ae` [2905] | An EMPTY terminfo capability is treated as undefined (#710). | ? |
| F58 | `3eb4d40` [2902] | `-z` may take a NEGATIVE value; the blanket ban on negative numeric options was wrong, hence `O_NEGOK`. | **OK** `-z-3` and `-z3` both accepted, matching less. |
...
| F61 | `d70ac51` [2848] | Option errors name the option with its `--` prefix: `There is no --xxx option`, `The --xxx option should not be followed by =`. | **FIX** we BUILT both messages correctly and never printed them when catting — `printStartupError` wrote unconditionally to stdout while less's `error()` uses the current output fd, which is stderr until `set_output(1)`. Verified all four cases against the binary. |
...
less REVERTED in 2845 — do not port.

## Found while probing, not from a commit message

| # | evidence | what differs | status |
|---|---|---|---|
| F64 (searched: NO less commit explains this — it is original design, not a missed fix, which confirms it belongs to the linebuf model rather than the audit) | dump of `/tmp/bin1.dat` under `-R` | **Attribute runs are per-character, not per-run.** less emits `ESC[7m ^@^A^B ESC[27m` and `ESC[7m <FF><FE> ESC[27m` — one standout run spanning adjacent same-attribute chars, because put_line/at_switch write a transition only when the attribute CHANGES. We emit `ESC[7m<FF>ESC[m ESC[7m<FE>ESC[m`, one wrap per character, because the transform wraps each representation independently in `colored('ctrl'/'bin', ...)`. | **GAP (cosmetic today)** the emulated screens are IDENTICAL — the pty differ is silent — so nothing is visibly wrong; this is byte-level output parity only. SAME ROOT CAUSE as the reverted representation-clustering work: less keeps attributes in the parallel `linebuf.attr` and emits transitions, we splice escape pairs into the text. Fix it with the linebuf reform, not separately. Note `coalesceOwnRuns` in helpers.ts already does exactly this merging for OVERSTRIKE runs, so the precedent and the shape of the fix exist. |

| F65 | measured, `short5.txt` + `G` | **`G` on a file SHORTER than the screen must move the content to the BOTTOM.** less's jump_forw ends in `jump_loc(pos, sc_height-1)`, placing the last line on the bottom screen line; there are not enough lines above, so the top rows become null lines and less draws `~`. We leave the content at the top with blank rows and never move. Visible in the pty differ (`less|~` vs `us|` on rows 0-1) and it is also why less's second `G` bells there and ours does not — after the move bottom+1 == end_pos. | **FIXED** — and my first root-cause call was WRONG. It is not the near-target `onscreen()` branch. The mechanism is further down `jump_loc`: it walks back `sindex` lines to place the last one at the bottom, and when `back_line` hits BOF first it BREAKS and passes the shortfall to `forw()` as its **nblank** argument — "rely on forw() below to draw the required number of blank lines at the top of the screen" (jump.c). So a short file ends up at the BOTTOM under tildes. Our `gotoEnd` clamped at BOF and left it at the top. Fixed by setting `padTop` to the shortfall. Exposed a second, latent bug in the process: `forward()` consumed a pad row before testing for EOF, while less's `forward()` asks `position(BOTTOM_PLUS_ONE)` and bells FIRST (forwback.c:481) — fixed too. Seven probes verified (GGG, G j b G, G j j b b, forced G J J, plus lines40/wrapped6/filtered). **Sweep seeds 1/2/3 are still open and still belong to the near-target branch** — that part of the earlier note stands. |
...
| 3 | 55 | RETURN | **step 14 FIXED.** Bisected to a 3-key minimum: RESIZE, `G`, RESIZE. Only the PROMPT differed — less `(END)`, us `:` — with every content row identical. Cause: `calculateEOF` decides `mode.EOF` from whether the CONTENT ARRAY fits one screen, but for a source engine that array is a multi-screen materialized window, so it always answers "no" and wiped the `mode.EOF` the engine's `sync()` had just derived from the file. less cannot have this bug: `eof_displayed` reads `position(BOTTOM_PLUS_ONE)` off its one table (forwback.c:95), so a resize that does not move the top cannot change the answer. Now diverges much later, at step 55. |
| 4 | 15 | RETURN | **CHARACTERISED, not fixed.** Minimal repro found by bisection: `j, RESIZE, k, /e⏎` (6 keys, down from 14). Both the RESIZE and the `k` are required — `f` in place of `k` is clean, and dropping either the leading `j` or the RESIZE is clean. Symptom: after the search less's row 0 runs `[014]..[027]` (a full 70-column row) and ours stops at `[014][015]` — exactly `[70,80)`, which is the SEAM extent the preceding `k` created (old top was char 80 at width 80; after the resize `k` lands on 70). So the seam entry survives the search in our render and less's does not. TWO HYPOTHESES TESTED AND BOTH WRONG: (1) clearing the table before `landMatch` repaints instead of after — no change; (2) forcing a full repaint on the landing — no change, which also rules out a redraw/prevRows artifact and proves the row is computed short. The stale extent therefore is not coming from `config.screen` via the paths I checked. INSTRUMENTED (the guessing stopped): `buildScreen` receives `config.screen = [{row:0, offset:70, end:80}]` on every frame from the `k` onward — the seam entry is never cleared. `landMatch` is NEVER REACHED, because the search FAILS: `chunks.txt` holds only `[0000]`-style tokens and the pattern is `e`, so both pagers report "Pattern not found". **less repaints anyway** — its frame for the failing RETURN is 3821 bytes where the two keys before it were 5 and 6 — and that repaint is `jump_loc(position(TOP), 1)`, which pos_clears and regenerates every row whole. We emit nothing and keep the seam.
THIRD ATTEMPT ALSO FAILED: clearing the table + re-syncing on the not-found path moved the divergence from row 0 to row 1 instead of removing it, and left every seed unchanged, so it was reverted too. Reading: replicating less here is not just "drop the seam" — less's repaint re-derives the whole screen from table[TOP] through a path we do not have an equivalent of on a non-moving search. **ANSWERED by `repaint_hilite` (search.c).** Two facts about less's table that we do not implement:
(1) **less's table stores STARTS ONLY** — the redraw loop is `forw_line(position(sindex)); goto_line(sindex); clear_eol(); put_line(FALSE)` with NO end bound, so a row's extent is recomputed every draw. A row `back_line` cut short at a seam is short only in the frame that drew it. Our `ScreenRow` carries its own `end` and keeps it forever.
(2) **less persists ALL sc_height entries.** We persist only the SEAM rows and let `buildScreen` regenerate everything below from the last entry's end.
Together they explain the measurement exactly: less redraws its STALE starts (70, 80, 150 — the 80 dates from the old width) at the new full width, so its rows genuinely OVERLAP, row0 = 70..140 AND row1 = 80..150. We have one seam entry, so row0 redraws to 70..140 and row1 continues at 140.
...
| 5 | 15 | `g` | NOT a near-target miss. The keys before it are `...2d,72,0a` — `-r` is ON, so the whole 11001-byte line is ONE screen row (`fits_on_screen` returns TRUE unconditionally, line.c:842). less ends with a screen of tildes, we show content. Same `-r` family as the styled gate's r-toggle/r-resize, i.e. the linebuf model, not the jump. |
...
| # | commit | claim | status |
|---|---|---|---|
| F66 | `2800` `16879fd` | `ESC-f`: like `ESC-F` but on a match it only rings the bell, staying in F mode. | **OK** `keys.ts:287` binds `\x1Bf` -> FOLLOW_BELL; less binds `ESC,'f'` -> A_F_FOREVER_BELL. |
...
| F70 | `2794` `62d87f4` | `ESC-u` must NOT clear the compiled search string — only hide/unhide the hilites. | **OK** probed `/01⏎ ESC-u ESC-u n`: identical to less, so the pattern survives the toggle and `n` still steps. |
| F71 | `2785` `57c2728` | Changing a STRING-valued option with `-` must print a confirmation, like numeric and boolean ones do. | **OK** probed `-Pfoo⏎`: identical to less. |
...
| F73 | `2787` `84b407f` | The `-j` ARGUMENT must be kept separate from the computed `jump_sline`, so a resize recomputes it from the original. | **OK** probed `-j.5` both orders — resize then search, and search then resize then `n`: identical to less, so the fractional argument survives and is recomputed at the new height. |
...
terminal-init experiment and its seven follow-up repairs — less REVERTED the
...
| # | commit | claim | status |
|---|---|---|---|
| F76 | `2760` `db2e844` | less REMOVES replace mode and its commands — "too complicated and risky for a feature that seems to be very rarely used and has never been requested". | **OK by absence** we never had it. Our only `'overwrite'` is the `-o` log-file prompt; less's cmdbuf.c has no `replace` at HEAD either. A chronological port would have built this and then deleted it. |
...
| F80 | `2746` `ceac046` | `ESC-u` with an OSC 8 highlight but no search pattern must clear the highlight, update the display, and NOT say "No previous regular expression". | **FIX** we showed less's PRE-fix message. less's `undo_search` runs `osc8_active = undo_osc8()` FIRST and gates the complaint on it: `else if (!osc8_active) error(...)` (search.c:405). Verified all three branches against the binary — link selected (quiet), pattern present (toggles), neither (still errors). |
| F81 | `2712` `342a086` | `--incsearch` returning to the start line on a non-match must restore the horizontal COLUMN too, not just the line. | **?** — probe inconclusive AND it exposed something else. With `--incsearch` on chunks.txt, two `ESC-)` shifts already diverge BEFORE any search: less's prompt reads `(END)`, ours `:`. So a horizontal shift changes less's eof_displayed and not ours (see F83); the incsearch claim itself is still untested behind it. |
| F82 | `2782` `cb0d379` | A NEGATIVE `-j` argument is normalized in calc_jump_sline, plus a check that jump_sline stays on screen. | ? |

### Adjudicated without a flag
`2775` ignaw -> defer_wrap: we already use DEFER_WRAP with less's two-meaning
...
| F83 | measured, `ESC-)` on a one-line file | Two `ESC-)` right-shifts and less's prompt becomes `(END)` while ours stays `:`; content rows agree. A horizontal shift changes less's `eof_displayed` answer and not ours. Same SHAPE as the resize-EOF bug fixed in `e12e089` — a second source of truth for "is the end displayed" going stale — but a different trigger, and not yet traced. | **FIXED** less's shifts all end in `screen_trashed()`, so the next make_display repaints and eof_displayed re-reads `position(BOTTOM_PLUS_ONE)` from the rebuilt table. It matters because a shift CHANGES a line's row count: `forw_line` is called with `chop_line() || hshift > 0` (input.c:348), so ANY hshift reads lines CHOPPED, one row each — less's screen after `ESC-)` on the one-line file is a single row ending in the rscroll `>` with tildes below, hence `(END)`. Our shift commands changed `config.col` and re-synced nothing; `posRehead()` early-returns when the top is already at a line start, so the source engine kept the EOF flag it had while the line was still folded. All four shift commands now resync. |

## From commits 2666-2709 (v698..v700 era)

| # | commit | claim | status |
|---|---|---|---|
| F84 | `2692` `77dddb2` | `LESS_TERMCAP_SUSPEND` / `LESS_TERMCAP_RESUME` wrap screen updates; less reads them via `ltgetstr` (screen.c:1596-99) because the sequences are in no termcap entry. | **FIX** we NAMED them in a comment and then emitted our own sync-update pair unconditionally, so the override did nothing. Now `terminalCapability(null,'SUSPEND'/'RESUME')` with our pair as the default. Verified: default emits `ESC[?2026h`, and with the vars set emits the custom strings instead. |
...
| F87 | `2673`+`2674`+`2670`+`2671` | Unicode: emoji modifiers DELETED from output, VS15 discarding a shifted-off double-width placeholder, more chars treated as binary. | **OK** the deletion half is F93 (fixed in `4aff42a`). The VS15 half probed clean: a file mixing VS15, VS16, bare wide chars and CJK agrees with less as displayed, and agrees under a right-shift, a shift-and-back, and a one-column narrowing — the cases where less's placeholder discard applies. A width mismatch would shift every following column, so the differ would catch it. |
...
| `-p` `--pattern` | **OK, verified by running** — `-p line-020` puts line-020 at the top in BOTH, so it is handled outside the spec like less's `opt_p` ungetting `/pattern⏎`. |
...
| F96 | `2633` `e66db83` | `^S` sub-search hangs if the pattern matches an empty string — the loop restarts from the same point. | **FIXED (the hang was never ours; the rule was wrong).** Probed `/ ^S 1 (x*) ⏎` on a 40-line file: neither pager hangs (less's own bug is fixed and we never had that loop). But less reports `Pattern not found: (x*)  (press RETURN)` and we print nothing, leaving the prompt at `:`. Content rows are identical, so neither moved — only the report differs. Traced to `testRegex`: it accepted a group that merely PARTICIPATED (`match[n] === undefined`) and judged only the FIRST match. less's `subsearch_ok` fails on `ep[i] == sp[i]` — the group must be NON-EMPTY — and `match_pattern` keeps searching AFTER each non-satisfying match, giving up when `mlen == 0` because it cannot advance (that guard is `e66db83` itself). Both rules implemented; verified the empty-group case, a `^S` group that genuinely matches, and a plain search, all against the binary. |
...
| # | commit | claim | status |
|---|---|---|---|
| F104 | `2566` `95f1d4d` | `-r` appearing in the `LESS` environment variable is treated as `-R`, because "it is almost always an error to put -r in the LESS environment variable". Command-line `-r` is unaffected. | **OK** probed `LESS=-r` on a styled file: identical to less. Discriminating, because under true `-r` our layout makes the whole line ONE row (fits_on_screen) and the screen would differ wildly; matching less means we take the `-R` path. |
| F105 | `2576` `bb4df5c` | `match_pattern` on an EMPTY line always returned FALSE, which is wrong for a NON-MATCH (`^N`) search — an empty line does not contain the pattern, so it matches. | **OK** probed `/ ^N aaa ⏎` over a file of alternating blank and non-blank lines: identical to less. |
| F106 | `2571` `b71a578` | (1) `prep_hilite` must NOT pass the SUBSEARCH bits to match_pattern — all parenthesized groups stay highlighted even when `^S` restricts the SEARCH; (2) a match rejected by SUBSEARCH must not fail the line, the search continues later in it. | **(2) FIXED** in `26ad91d`. **(1) OK by measurement.** I expected a divergence from reading the code (our highlight path pushes only the `^S` groups' spans) but three probes disagree: `^S1` with `(alpha).*(gamma)` matches less plainly, with `--use-color` (where the per-group subsearch colours would show), and with `^S2`. Whatever less does after `prep_hilite` drops the SUBSEARCH bits, the painted result is the same as ours. Reading the code predicted a bug the binary does not have — probe before believing a code-read. |
...
| F114 | `2556` `52b2d92` | With `-F` on a short file, do not poll the input - polling eats tty input the user expects to reach the shell. | **OK - N/A by construction, verified by measurement.** less's hazard is `check_poll` (os.c:158) polling the tty alongside the input fd and doing a real `getchr()` on POLLIN; we never poll an fd, so there is nothing to eat. Probed anyway with a type-ahead harness (`typeahead.py`: writes `MARKER\n` to the pty BEFORE the pager starts, then `dd bs=1 count=7` after it exits): SURVIVED for less and for us, on a regular file, a pipe and a slow pipe. Controls confirm the probe discriminates - a `dd bs=1 count=1` stand-in reports EATEN, a `true` reports SURVIVED. |
...
| F116 | `2523` `36da1f9` | `init_textlist` misbehaves when the string ends in a space, so TAB completion after `--` segfaulted; `findopts_name` strips the trailing space. | **The crash is not ours (no textlist), but probing it found a REAL GAP — FIXED** in `2f53674`. TAB cycling after `--h` gave us hilite-search, hilite-unread, help, header, hilite-target where less gives hilite-search, hilite-target, hilite-unread, help, header: `findopts` walks the option TABLE in order, and ours had drifted from opttbl.c in 69 of 85 shared positions, with later additions simply appended. Re-derived the array entry for entry. The order also decides which option an ambiguous `--` prefix resolves to, so this was never only about completion. |
| F126 | (not a commit — found comparing the tables) | less's table has `--tty`, an `O_STRING\|O_NO_TOGGLE` option naming the tty to read input from (`opt_ttyin_name`, opttbl.c:203). | **NOT A GAP — my extraction was wrong.** The entry sits inside `#if LESSTEST` (opttbl.c:814), so it is compiled only into a test build: `ttyin_name` and `is_lesstest()` are guarded the same way. The shipped binary answers `There is no --tty option`, and so do we, identically. Lesson: read the option table with its #ifdefs, or ask the binary. |
| F117 | (not a commit — found probing this batch) | less's `edit_ifile` takes the name `-` to mean fd0 (edit.c:516). | **FIXED** in `036b37e` (alone) and `c05ffb4` (in a list). Standard input now spools to a private file up front and the entry points at it, so the block engine sees an ordinary growing file - less keeps fd0 with CH_KEEPOPEN and buffers it in ch; the spool is our ch. Verified with `-` first, last and in the middle of a three-file list, :n onto it and :p off it. |
| F118 | (not a commit — found probing this batch) | `bad_file`'s second guard is `!S_ISREG`, not isatty. | **WAS A REAL GAP — FIXED** in `519521f`. We refused character devices with edit.c's "is a terminal" message; less refuses every non-regular file with "is not a regular file (use -f to see it)", and a fifo did not even reach a message (the open blocked). |
...
| F122 | `2511` `5a369ed` | `ap_char` must insert the raw byte; routing it through `ap_str` corrupts UTF-8 in a `-P` prompt, because ap_str now converts non-printables and the prompt's multibyte chars arrive one byte at a time. | **OK on the UTF-8 half, but it exposed a REAL GAP on the other half — FIXED** in `80db8f1`. Our UTF-8 prompts already matched (accents, arrows, CJK, check marks — five probes). But the reason less can store the prototype's bytes raw is that the conversion happens LATER, at load_line, and we had no such step: a literal BEL in a -P rang the terminal instead of showing `^G`, `^A`/`^B` vanished, and a TAB moved the cursor and spilled the prompt onto a second row. Two prompt-specific rules came with the fix: do_append treats a NULL_POSITION char as ANSI-live regardless of ctldisp (line.c:1302 — less carets an ESC in file content at the default ctldisp but passes the same bytes through in the prompt), and load_line skips the standout colouring when the line has ANSI of its own (line.c:1950). |
...
| F125 | `2500` `0df5d8b` | lesskey gains `mouse` and `mouse6` commands; mouse parameters must not be read from the tty until every command table is parsed. | **FIXED** in `5422566`, and it uncovered a bigger gap: less reads mouse reports in TWO wire formats and we implemented only SGR, so a terminal reporting in the older X10/X11 three-byte form did nothing in our pager while less scrolled. Both formats now decode. The lesskey names resolve to the two actions in both tables. Remaining: the SGR introducer is still matched by shape, so a lesskey file cannot MOVE it (F144); the X11 one is rebindable. |

## From commits 2461-2497 (v661..v668 era)

| # | commit | claim | status |
|---|---|---|---|
| F127 | `2497` `03f9f1a` | A LESSOPEN script whose first byte has the high bit set sign-extends into `ch_ungetchar`, which reads a negative value as "discard the queued char". | **The sign extension is not ours, but probing it found a REAL GAP — FIXED** in `0a231d4`. A valid multibyte first byte already matched. An INVALID one did not: both LESSOPEN paths used `toString('utf8')`, so bytes that are not valid in the charset became U+FFFD and we printed `<U+FFFD>` where less prints `<FF>`. A plain file and a piped stdin were already right, so it was specific to the preprocessor. |
...
| F131 | `2471` `56fb53f` + `2470` `994786e` | `LESS_SHELL_LINES` reserves shell rows in the `-F` fits test, and its init had to move to `get_term` because `get_one_screen` is not called under `-X`. | **OK** — the discriminating case is a 22-line file in a 24-row window: with `LESS_SHELL_LINES=3` less PAGES, with `=1` it exits. Ours matches on that case with `-F -X`, with `-F`, and unset. |
...
| F137 | (not a commit — found probing F136) | Under `-r` ONLY, a first screenful containing a raw cursor-moving escape (here `ESC D`, index) lands one row off ours. | **FIXED** in `3fbdae0`. less's squished first screen is not painted from the top at all: term_init leaves the cursor on the BOTTOM line and each drawn line's newline scrolls the short file up. Painting it row by row lands identically for ordinary text - which is why it went unnoticed - but not for content that moves the cursor itself. Three more fell out of the chase: (a) under `-r` less counts NO widths (`fits_on_screen` returns TRUE outright for `ctldisp == OPT_ON`), so the prompt is never clipped and wraps instead; (b) a zero-byte file draws no line and less's prompt() then clear_bots, where a single-NEWLINE file does draw one; (c) `scrolledFrame` could NEVER fire for the block-backed engine, because less reads the scroll distance off its position table while `config.row` stays 0 when the WINDOW moves - so every scroll was a full repaint, invisible until a drifted screen showed the repaint resetting drift less carries. |
| F138 | `2443` `f9e5b43` | `--header` cannot be parsed at INIT (find_pos needs the open file), so less saves the string and applies it as a TOGGLE after the first open. Fixes a crash with three arguments. | **WAS A REAL GAP — FIXED** in `3453df9`. We deferred it like less but then applied the start TWICE - once inside setHeader (through the source engine's seek, which rebuilds its window and resets config.row) and again in applyPendingHeader - so `--header=2,3,10` opened at line 21 where less opens at 12. A columns-only `--header=0,C,N` also moved the view, where less's set_header stores NULL_POSITION for the start when header_lines is 0. |
...
| F140 | `2391` `1649cc3` | lesskey_parse fixes, including the filename reported for a `--lesskey-content` syntax error. | **The filename already matched, but the probe found TWO REAL GAPS — FIXED** in `81e5a31`. less's parse_lesskey returns an ERROR COUNT and the option handlers add a summary line ("Error in lesskey content", "Cannot use lesskey source file"); ours threw the count away, and the src handler only reported a file it could not READ. Separately, `$LESSKEY_CONTENT` reported its error twice because we parse the lesskey files twice - the CLI needs the `#env` lines before it can classify argv - so the CLI's pass is now quiet. |
| F141 | (not a commit — found probing F140) | less's `quit()` clear_bots the prompt line before term_deinit, on any screen. | **WAS A REAL GAP — FIXED** in `2d7bb3f`. We gated it on `-X`. Only visible to a probe that quits MID-SEQUENCE: every ordinary capture ends with the harness's trailing `q`, sent after the last recorded step. A lesskey `Z quit` binding made the quit a captured step and exposed it. |
...
| # | what | detail |
|---|---|---|
| F142 | our forward scroll uses an explicit scroll-up | **FIXED** in `4a5fa49`, and the prompt-write half in `1e954fc`. less scrolls with NEWLINES, never `ESC[nS`/`ESC[nT`; `clear_bot` uses `line_left()` (a bare CR to column 1 of wherever the cursor is), never an absolute address; and `forw_prompt` makes prompt() skip clear_bot entirely after a forward move. `j` and `k` are now byte-identical to less apart from our `ESC[?2026` sync wrapper. |
| F143 | our startup emits `ESC[?1007h` and an OSC title | **FIXED** in `a55a617`. less has no `?1007` anywhere and titles only on WIN32, through SetConsoleTitleW and to the FILE's name. The same commit made the first paint sequential from the bottom line for a FULL screen too, not just a short one - it is term_init's lower_left, not the squish. |
| F144 | (not a commit — left by the F125 fix) | A lesskey file can name `mouse6` but cannot move the SGR introducer. | **FIXED** in the F144 commit. less binds only the INTRODUCER and its decoder reads the report's bytes with getcc, so the length is discovered; ours collects the pre-split keys after the introducer and completes at three bytes (X11) or M/m after three numeric fields (SGR). It also exposed a pre-existing one: a mouse report that does NOTHING still returns less to prompt(), so the prompt is reprinted - visible on the first report, when the filename prompt gives way to `:`. |
| F145 | (not a commit — recorded then explained from the source) | On a file with NO lines, a command that only bells still repaints less's screen into tildes. | **FIXED** in `eb9800b`. `make_display()` opens with `if (empty_screen()) jump_loc(ch_zero(), 1)` and prompt() calls it before every prompt (command.c): a file with no lines never fills the position table, so it repaints forever. A 12-line file does not, because its table is full - the rule keys on the TABLE, not on the move, which is why "unsquish on every move" was wrong. make_display runs from prompt(), so an MCA loop (a `/` prompt) never reaches it. |

## From commits 2254-2387 (v643..v653 era)

| # | commit | claim | status |
|---|---|---|---|
| F146 | `2384` `bf14bc4a` | Only FILENAMES in a prompt are converted to printable form, not the whole prompt — converting everything broke escape sequences in `-P` strings. | **WAS A REAL GAP — FIXED** in `1dfe605`. less passes `%f`/`%F` through `ap_estr(..., TRUE)` = prutfchar at BUILD time and everything else through `ap_str`, which converts nothing (prompt.c:337). prutfchar is NOT the draw-time conversion: it renders a tab as `^I` where the line machinery expands it to a tab stop. A file named `tab<TAB>name` showed as `tab^Iname` in less and `tab` + eight spaces in ours. Nothing would have made me probe a tab in a filename. |
| F147 | `2375` `855ab5cc` | `updown_match` must be saved AND restored around an incremental search, or history scrolling breaks. | **OK on the substance — but the probe exposed F148.** Our history recall survives `--incsearch` (probed with two prior patterns, a prefix, and Up/Up/Down). |
| F148 | (not a commit — found probing F147) | less binds ONE sequence per special key, resolved from terminfo (`SK_UP_ARROW` → `ltgetstr("kcuu1","ku")`, screen.c:1226). With the keypad init sent, an xterm's up arrow is `ESC O A`, and a bare `ESC [ A` is not an arrow at all — less inserts it as three characters. We bind BOTH forms. | **OPEN, deliberate.** Probed: less echoes `/rESC[A` where we recall history. Making it strict needs the terminfo value, and we have no terminfo reader — `terminalCapability` only consults `$TERMCAP` and `LESS_TERMCAP_*`. Hardcoding the xterm application form instead would be WRONG on any terminal whose `ku` really is `ESC [ A`, so the permissive map stays until there is a real terminfo lookup. The divergence it leaves is typing a literal `ESC [ A` as search text. |
| F149 | `2285` `1fafd968` | Use `read()`, not `statfs()`, to detect files that lie about their size — procfs and tracefs report 0 bytes but return data. | **N/A here, real on Linux.** macOS has no procfs, so the case cannot be constructed to probe. Our loader trusts the stat size, so a 0-byte-but-readable file would show as empty. Worth revisiting if Linux support is ever exercised. |
| F150 | `2387` `f752294d` | `fexpand` buffer overflow by one. | **N/A** — C allocation arithmetic. |
| F151 | `2322`/`2314`/`2313`/`2312`/`2363` | Windows SGR processing, console title encoding, DJGPP build. | **N/A** — Windows/DOS build paths. |
| F152 | `2286` `19679de6` | Do not assume `PATH_MAX` is defined (GNU/Hurd). | **N/A** — no fixed path buffers. |
