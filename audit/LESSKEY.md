...
| # | status | case | less | ours |
|---|---|---|---|---|
| K01 | **KEPT** | `x digit` bound, then pressing `x` | spins | the key joins the count |

less cannot leave that state. `fcmd_decode` returns `A_DIGIT`, the loop
...
less's own A_DIGIT arm would treat a digit and moves on.
...
| # | status | case | was | now |
|---|---|---|---|---|
| K03 | **FIX** | the four mouse codes (66, 67, 78, 79) in a compiled file | rang — we had no action to map them onto | the wheel moves, like less's `A_F_MOUSE` (`command.c:1720`) |
| K04 | **FIX** | `goto-pos`, and code 51 | rang — the table said unsupported | jumps to the byte position, like less's `A_GOPOS` (`command.c:1918`) |
...
| K06 | **FIX** | `--lesskey-help` said `debug` did nothing | less writes no `case A_DEBUG`, so it falls to `commands()`'s default and RINGS (`command.c:2517`) | the page says it rings |
...
proves nothing: `P` is already `A_GOPOS` in less's built-in table
...
already taken. With `x goto-pos`: less and ours both land on line 39 with
...
because they are what less's DECODER resolves a wheel report to
...
> the file was nowhere near. Measured against less: three wheel-downs
> reach line 26 in less and line 24-then-bell in ours.
>
> less has no such split — a wheel report becomes an action and runs
...
> **A report during a prompt leaked into the prompt.** less reads it
...
>
> Measured: with `:5` up, less swallows a wheel tick whole and still
> shows `:5` at exit. `/li` + a tick + `ne 9` searches for `line 9`
> in less; ours searched for something with `[<65;1;1M` in it.

## Verified matching — do not re-investigate

Every one of these was byte-compared or screen-compared against less and
came out identical.

| case | note |
|---|---|
| `#version` guards | `>= 707`, `> 600`, `= 707` all true; `< 707`, `>= 999` false. We compare 707, like less, not our npm version |
...
| `invalid` vs `noaction` | less rings the bell for `invalid` only; ours matches (BEL 1 vs 0) |
...
node starts slower than less. At the default, our pager often misses the
