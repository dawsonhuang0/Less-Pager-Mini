 ...
 *
 * og suppresses the prompt through check_poll (os.c:136): a read that
...
describe('the ":" hold', () => {
  ...
  it('does not engage without a key waiting', () => {
    ...
    // single slow command hid the prompt with nobody behind it - og
  ...
  it('does not engage on a backlog alone', () => {
    ...
    // took the ":" off a one-screen file where og's stays put.
    markBurst(true);
    markCommandTime(0);
  ...
  it('never engages on the help file', () => {
    // og's ch answers CH_HELPFILE from memory (ch.c:616): it is never
  ...
  it('releases at an edge, so (END) is written mid-burst', () => {
    // og's forw() with nlines == 0 bells and returns without reading
  ...
  it('cannot re-arm while the edge still holds', () => {
    ...
    // og writes its prompt for all of them. Re-arming between them is
    // what flickered "(END)".
    markStalled();
    markBurst(true);
    markCommandTime(500);
