...
| F137 | (not a commit — found probing F136) | Under `-r` ONLY, a first screenful containing a raw cursor-moving escape (here `ESC D`, index) lands one row off ours. | **OPEN, narrow.** The content bytes are byte-identical; the difference is the SQUISH shape. og positions the cursor at the bottom line and lets `\r\n` scroll the short file up; we clear N rows and draw. The two normally coincide - verified on ordinary files - and only diverge when the content itself moves the cursor, which needs `-r`. Fixing means re-shaping the first paint, which the gate depends on; not attempted at the tail of a batch. |
