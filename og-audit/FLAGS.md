...
| # | commit | claim | status |
|---|---|---|---|
| F115 | `2526` `23ff6a4` | Any key, not just ^C/^X, interrupts a file read. Marked "an experiment". | ? — check whether 707's `check_poll` still gates on `intr_char` (it does, os.c:167), i.e. whether this was walked back. |
| F116 | `2523` `36da1f9` | `init_textlist` misbehaves when the string ends in a space, so TAB completion after `--` segfaulted; `findopts_name` strips the trailing space. | ? |
...
| F119 | `2519` `132e657` | `LESS_SIGUSR1` / `LESS_SIGUSR2` environment variables name a command to run on those signals. | ? |
| F120 | `2518` `37cfa16` | A signal during a tty read makes `getchr` return READ_INTR, and `getccu` turned that into a spurious `'\0'` command char; it must ignore it and read another. | ? |
| F121 | `2516` `9c7105e` | An option error message still said `--` where the syntax became `-!` in 1999. | ? — cheap to probe, compare our text. |
| F122 | `2511` `5a369ed` | `ap_char` must insert the raw byte; routing it through `ap_str` corrupts UTF-8 in a `-P` prompt, because ap_str now converts non-printables and the prompt's multibyte chars arrive one byte at a time. | ? |
| F123 | `2509` `4e516a1` | `--no-poll` added. | **N/A — reverted upstream.** 707 says "There is no --no-poll option", and so do we. |
| F124 | `2501` `979aeab` | Reading from a pipe, `ch_fsize` must stay NULL_POSITION; two paths set it without checking CH_CANSEEK. | ? — bears on our `sizeKnown`. |
| F125 | `2500` `0df5d8b` | lesskey gains `mouse` and `mouse6` commands; mouse parameters must not be read from the tty until every command table is parsed. | ? |
