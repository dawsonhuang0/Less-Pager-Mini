...
| F126 | (not a commit — found comparing the tables) | og's table has `--tty`, an `O_STRING\|O_NO_TOGGLE` option naming the tty to read input from (`opt_ttyin_name`, opttbl.c:203). | **NOT A GAP — my extraction was wrong.** The entry sits inside `#if LESSTEST` (opttbl.c:814), so it is compiled only into a test build: `ttyin_name` and `is_lesstest()` are guarded the same way. The shipped binary answers `There is no --tty option`, and so do we, identically. Lesson: read the option table with its #ifdefs, or ask the binary. |
| F117 | (not a commit — found probing this batch) | og's `edit_ifile` takes the name `-` to mean fd0 (edit.c:516). | **FIXED** in `036b37e` (alone) and `c05ffb4` (in a list). Standard input now spools to a private file up front and the entry points at it, so the block engine sees an ordinary growing file - og keeps fd0 with CH_KEEPOPEN and buffers it in ch; the spool is our ch. Verified with `-` first, last and in the middle of a three-file list, :n onto it and :p off it. |
...
| F125 | `2500` `0df5d8b` | lesskey gains `mouse` and `mouse6` commands; mouse parameters must not be read from the tty until every command table is parsed. | **FIXED** in `5422566`, and it uncovered a bigger gap: og reads mouse reports in TWO wire formats and we implemented only SGR, so a terminal reporting in the older X10/X11 three-byte form did nothing in our pager while og scrolled. Both formats now decode. The lesskey names resolve to the two actions in both tables. Remaining: the SGR introducer is still matched by shape, so a lesskey file cannot MOVE it (F144); the X11 one is rebindable. |
...
| # | what | detail |
|---|---|---|
| F142 | our forward scroll uses an explicit scroll-up | **FIXED** in `4a5fa49`, and the prompt-write half in `1e954fc`. og scrolls with NEWLINES, never `ESC[nS`/`ESC[nT`; `clear_bot` uses `line_left()` (a bare CR to column 1 of wherever the cursor is), never an absolute address; and `forw_prompt` makes prompt() skip clear_bot entirely after a forward move. `j` and `k` are now byte-identical to og apart from our `ESC[?2026` sync wrapper. |
| F143 | our startup emits `ESC[?1007h` and an OSC title | **FIXED** in `a55a617`. og has no `?1007` anywhere and titles only on WIN32, through SetConsoleTitleW and to the FILE's name. The same commit made the first paint sequential from the bottom line for a FULL screen too, not just a short one - it is term_init's lower_left, not the squish. |
| F144 | (not a commit — left by the F125 fix) | A lesskey file can name `mouse6` but cannot move the SGR introducer. | **FIXED** in the F144 commit. og binds only the INTRODUCER and its decoder reads the report's bytes with getcc, so the length is discovered; ours collects the pre-split keys after the introducer and completes at three bytes (X11) or M/m after three numeric fields (SGR). It also exposed a pre-existing one: a mouse report that does NOTHING still returns og to prompt(), so the prompt is reprinted - visible on the first report, when the filename prompt gives way to `:`. |
| F145 | (not a commit — recorded then explained from the source) | On a file with NO lines, a command that only bells still repaints og's screen into tildes. | **FIXED** in `eb9800b`. `make_display()` opens with `if (empty_screen()) jump_loc(ch_zero(), 1)` and prompt() calls it before every prompt (command.c): a file with no lines never fills the position table, so it repaints forever. A 12-line file does not, because its table is full - the rule keys on the TABLE, not on the move, which is why "unsquish on every move" was wrong. make_display runs from prompt(), so an MCA loop (a `/` prompt) never reaches it. |

## From commits 2254-2387 (v643..v653 era)

| # | commit | claim | status |
|---|---|---|---|
| F146 | `2384` `bf14bc4a` | Only FILENAMES in a prompt are converted to printable form, not the whole prompt — converting everything broke escape sequences in `-P` strings. | **WAS A REAL GAP — FIXED** in `1dfe605`. og passes `%f`/`%F` through `ap_estr(..., TRUE)` = prutfchar at BUILD time and everything else through `ap_str`, which converts nothing (prompt.c:337). prutfchar is NOT the draw-time conversion: it renders a tab as `^I` where the line machinery expands it to a tab stop. A file named `tab<TAB>name` showed as `tab^Iname` in og and `tab` + eight spaces in ours. Nothing would have made me probe a tab in a filename. |
