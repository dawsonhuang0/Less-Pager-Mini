...
| F65 | measured, `short5.txt` + `G` | **`G` on a file SHORTER than the screen must move the content to the BOTTOM.** og's jump_forw ends in `jump_loc(pos, sc_height-1)`, placing the last line on the bottom screen line; there are not enough lines above, so the top rows become null lines and og draws `~`. We leave the content at the top with blank rows and never move. Visible in the pty differ (`og|~` vs `us|` on rows 0-1) and it is also why og's second `G` bells there and ours does not — after the move bottom+1 == end_pos. | **GAP — ROOT CAUSE FOUND** it is NOT a `gotoEnd` bug. og's `jump_loc` has a near-target branch (jump.c:337): when `onscreen(pos) >= 0` the target is already displayed, so og SCROLLS to it with `forw`/`back` and **force=TRUE** instead of repainting. For `G` on a short file the last line is on screen at row 4, `sindex = sc_height-2 = 22`, so `nline -= sindex` gives `back(18, force=TRUE)` — a FORCED backward scroll, which pads null lines at BOF and draws them as `~`. We always repaint, so the content never moves. Same branch is the known cause of sweep seeds 1/2/3. Fix jump_loc's near-target branch and F65 + those three seeds go together. We own the padding machinery (`padTop`/`blankTop`). |
