...
| F96 | `2633` `e66db83` | `^S` sub-search hangs if the pattern matches an empty string — the loop restarts from the same point. | **PARTIAL: the hang is NOT ours, but a message is missing.** Probed `/ ^S 1 (x*) ⏎` on a 40-line file: neither pager hangs (og's own bug is fixed and we never had that loop). But og reports `Pattern not found: (x*)  (press RETURN)` and we print nothing, leaving the prompt at `:`. Content rows are identical, so neither moved — only the report differs. Not yet traced; our `^S` sets `search.subs` and something downstream is treating the empty-group case as a silent no-op rather than a failed search. |
