...
| F144 | (not a commit — left by the F125 fix) | A lesskey file can name `mouse6` but cannot move the SGR introducer. | **OPEN, narrow.** Our splitter claims `ESC [ <` by shape; a rebound prefix would need the report accumulated key by key after the introducer, which is exactly what og's `getcc` inside x116mouse_action does. The X11 introducer IS rebindable, since its length is fixed. |
